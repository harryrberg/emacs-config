...
| 2 | Launch the *MSYS2 MINGW64* terminal (not MSYS or UCRT64) and run the update command below. Restart the terminal if prompted and repeat until the system is fully updated. |
|    | #+BEGIN_SRC bash
pacman -Syu
#+END_SRC |
...
#+END_SRC |
| 4 | Install Emacs and the native compilation library. This installs Emacs with GUI support and `libgccjit` for native compilation. |
|    | #+BEGIN_SRC bash
