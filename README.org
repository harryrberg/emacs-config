...
* Minimal bootstrap: =~/.emacs.d/init.el=
Place the following in your =init.el=. It adds a simple after-save hook that tangles =~/.emacs.d/init.org= whenever you save it.
...
* Org config file: =~/.emacs.d/init.org=
Create this Org file with the same tangle-on-save snippet inside a source block so it is preserved when tangling.
